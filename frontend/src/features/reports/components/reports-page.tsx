...
import { downloadSessionsCsv, downloadSessionsPdf } from "@/features/reports/api/reports-api";
...
export function ReportsPage() {
  ...
  const timeseriesQuery = useQuery(reportsQueries.timeseries());
  const departmentsQuery = useQuery(reportsQueries.departments());
  const sessionsQuery = useQuery(reportsQueries.sessions(sessionParams));

  const summary = summaryQuery.data ?? null;
  const series = timeseriesQuery.data ?? [];
  const departments = departmentsQuery.data ?? [];
  const sessions = sessionsQuery.data?.results ?? [];
  const sessionCount = sessionsQuery.data?.count ?? 0;
  const tableLoading = sessionsQuery.isLoading;

  const loadError =
    summaryQuery.error ?? timeseriesQuery.error ?? sessionsQuery.error ?? departmentsQuery.error;
  const displayError = error ?? (loadError ? formatApiError(loadError) : null);
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={handleExportCsv} disabled={exporting !== null}>
